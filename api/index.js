...
const validUsers = {
    "Mohamed": "Mohamed", "Abas": "Abas", "Jaber": "Jaber", "Kamel": "Kamel", "Majed": "Majed", "Mohamed Ali": "Mohamed Ali", "Morched": "Morched", "Saeed": "Saeed", "Sami": "Sami", "Sylvano": "Sylvano", "Tonga": "Tonga", "Youssef": "Youssef", "Zine": "Zine"
};
...
// ===== ROUTE LOGIN MISE À JOUR (PLUS ROBUSTE) =====
app.post('/api/login', (req, res) => {
    const { username, password } = req.body;
    if (!username || !password) {
        return res.status(400).json({ success: false, message: 'Nom d\'utilisateur et mot de passe requis' });
    }
    // Recherche de l'utilisateur sans tenir compte de la casse (majuscule/minuscule)
    const userKey = Object.keys(validUsers).find(key => key.toLowerCase() === username.toLowerCase());

    if (userKey && validUsers[userKey] === password) {
        // Renvoie le nom d'utilisateur avec la bonne casse
        res.status(200).json({ success: true, username: userKey });
    } else {
...
// ===== TOUTES LES FONCTIONS DE GÉNÉRATION SONT ICI =====
...
app.post('/api/generate-word', async (req, res) => {
    try {
        ...
        if (datesNode?.start && datesNode?.end) {
            ...
            if (!isNaN(startD.getTime()) && !isNaN(endD.getTime())) { plageSemaineText = `du ${formatDateFrenchNode(startD)} à ${formatDateFrenchNode(endD)}`; }
        }
...
app.post('/api/generate-excel-workbook', async (req, res) => {
    try {
        ...
        const formattedData = planDocument.data.map(item => { const row = {}; finalHeaders.forEach(header => { const itemKey = findKey(item, header); row[header] = itemKey ? item[itemKey] : ''; }); return row; });
...
app.post('/api/generate-ai-lesson-plan', async (req, res) => {
    res.status(501).json({ message: "Fonctionnalité IA non entièrement implémentée dans cette version."})
});
